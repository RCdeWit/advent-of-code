...
with open("input.txt", "r") as file:
    ...
    new = lambda x, y: data[0][
        int(
            "".join(
                [binary[grid[(x + a, y + b)]]() for b in [-1, 0, 1] for a in [-1, 0, 1]]
            ),
            2,
        )
    ]
    for i in range(1, 51):
        ...
        minx, maxx = (
            min(z[0] for z in grid.keys()) - 1,
            max(z[0] for z in grid.keys()) + 2,
        )
        miny, maxy = (
            min(z[1] for z in grid.keys()) - 1,
            max(z[1] for z in grid.keys()) + 2,
        )
